...
class Player(db.Model):
    __tablename__ = "cube_jump_player"
    ...
    refresh_tokens = db.relationship("CJRefreshToken", backref="player", lazy=True)


class CJRefreshToken(db.Model):
    __tablename__ = "cube_jump_refresh_token"

    id = db.Column(db.Integer, primary_key=True)
    token = db.Column(db.String(255), unique=True, nullable=False, index=True)
    expires_at = db.Column(db.String, nullable=False)
    player_id = db.Column(db.Integer, db.ForeignKey("cube_jump_player.player_id"), nullable=False)
