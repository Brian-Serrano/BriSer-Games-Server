...
from room_escape_utils.database import Player, RERefreshToken
from room_escape_utils.utils import validate_username, validate_email, validate_password, create_access_token, \
    create_refresh_token
from config import db, limiter

authorization_routes = Blueprint("re_authorization_routes", __name__)
...
def refresh():
    ...
    stored_token = RERefreshToken.query.filter_by(token=token).first()
    if not stored_token:
