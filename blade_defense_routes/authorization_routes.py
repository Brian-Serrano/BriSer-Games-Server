...
from blade_defense_utils.database import Player, BDRefreshToken
from blade_defense_utils.utils import validate_username, validate_email, validate_password, create_access_token, \
    create_refresh_token
from config import db, limiter

authorization_routes = Blueprint("bd_authorization_routes", __name__)
...
def refresh():
    ...
    stored_token = BDRefreshToken.query.filter_by(token=token).first()
    if not stored_token:
