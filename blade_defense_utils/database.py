...
class Player(db.Model):
    __tablename__ = "blade_defense_player"
    ...
    refresh_tokens = db.relationship("BDRefreshToken", backref="player", lazy=True)


class BDRefreshToken(db.Model):
    __tablename__ = "blade_defense_refresh_token"

    id = db.Column(db.Integer, primary_key=True)
    token = db.Column(db.String(255), unique=True, nullable=False, index=True)
    expires_at = db.Column(db.String, nullable=False)
    player_id = db.Column(db.Integer, db.ForeignKey("blade_defense_player.player_id"), nullable=False)
