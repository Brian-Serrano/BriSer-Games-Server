...
class Player(db.Model):
    ...
    refresh_tokens = db.relationship("TRRefreshToken", backref="player", lazy=True)


class TRRefreshToken(db.Model):
    __tablename__ = "td_rubix_refresh_token"

    id = db.Column(db.Integer, primary_key=True)
    token = db.Column(db.String(255), unique=True, nullable=False, index=True)
    expires_at = db.Column(db.String, nullable=False)
    player_id = db.Column(db.Integer, db.ForeignKey("td_rubix_player.player_id"), nullable=False)
