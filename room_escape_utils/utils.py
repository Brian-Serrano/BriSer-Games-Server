...
import config
from room_escape_utils.database import RERefreshToken
...
def leaderboard_to_response(player, i, leaderboard_type):
    amount = 0

    if leaderboard_type == "coins":
        amount = player.total_coins
    if leaderboard_type == "level":
        amount = player.level
    if leaderboard_type == "high_score":
        amount = player.high_score
...
def create_refresh_token(player_id):
    ...
    RERefreshToken.query.filter_by(player_id=player_id).delete()

    refresh_token = RERefreshToken(
        token=token,
        player_id=player_id,
